...
import { ListWallets, WalletsConfig, WalletType } from "@src/configs/web3/wallets";
...
const WalletModal: React.FC<IWalletModal> = ({
  ...
  const { handleConnect: handleConnectPhantom } = usePhantom();
  ...
  const handleConnectWallet = async (wallet: WalletsConfig): Promise<void> => {
    try {
      ...
      } else if (wallet.type === WalletType.Solana) {
        // login with solana and phantom wallet
        setTimeout(() => {
          try {
            const web3Modal = document.getElementsByTagName("w3m-modal");
            if (web3Modal) {
      ...
      <ModalContent className={"p-4"}>
        ...
        >
          ...
          >
            ...
            <Icon
              viewBox="0 0 15 14"
              className={`transition duration-150 ease-in-out ${isExpanded ? "rotate-180" : ""
                }`}
            >
              <path
